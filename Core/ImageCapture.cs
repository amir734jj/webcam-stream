...
namespace WebcamStream
{
    public class ImageCapture : IDisposable
    {
        ...
        public async Task Run(string selectedDevice, uint width, uint height, TimeSpan between)
        {
            ...
            // Listen to events
            this._processingCancellationTokenSource = new CancellationTokenSource();
            mediaPlayer.Stopped += delegate { _processingCancellationTokenSource.CancelAfter(1); };

            // Create new media
            using var media = new Media(libVlc, selectedDevice, FromType.FromLocation);
            ...
            try
            {
                await ProcessThumbnails(_processingCancellationTokenSource.Token);
            }
